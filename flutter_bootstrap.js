...
// Optimize and bootstrap Flutter Web with high performance
_flutter.loader.load({
  config: {
    // Force the HTML renderer by default to achieve a 'flash-like' instant loading speed.
    // The HTML renderer uses standard DOM elements, which reduces the bundle size by avoiding
    // canvaskit.wasm (saving ~2.5MB of downloads) and dramatically cuts down startup time.
    renderer: "html",
  },
  ...
  onEntrypointLoaded: async function(engineInitializer) {
    ...
    // Smoothly fade out the HTML loading screen to create a premium, high-end transition
    const loadingDiv = document.getElementById("loading");
    if (loadingDiv) {
      loadingDiv.style.transition = "opacity 0.35s cubic-bezier(0.4, 0, 0.2, 1)";
